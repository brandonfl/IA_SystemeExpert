...
dardevil :- negatif(animation), adaptation, super_heros.
...
stranger_things :- n:- unknown(trace,fail).
:- dynamic(negatif/1).
...
black_mirrow :- negatif(animation), negatif(adaptation), negatif(americain), science_fiction, dystopie.
